...
export default function MultimodalDiagnosis() {
  ...
  const validateFileType = (file: File | null, acceptedTypes: string[]): boolean => {
  ...
  const renderFileUpload = (
    ...
    const isValidFile = validateFileType(file, acceptedTypes);
    ...
    return (
      <div className="space-y-2">
        ...
        {file && !isValidFile && (
          <p className="text-sm text-red-500">Invalid file type. Accepted types: {acceptedTypes.join(', ')}</p>
        )}
        {file && isValidFile && <p className="text-sm text-muted-foreground">File: {file.name}</p>}
      </div>
    );
  };
        ...
        <Tabs defaultValue="cnn" className="w-full">
          ...
          <TabsContent value="cnn">
            <Card>
              ...
              <CardContent>
                {renderFileUpload("CNN Image", "image/*", <FileUp className="h-6 w-6" />, cnnFile, setCnnFile, ["image/jpeg", "image/png", "image/svg+xml"])}
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="lstm">
            <Card>
              ...
              <CardContent>
                {renderFileUpload("LSTM Data", ".csv", <Activity className="h-6 w-6" />, lstmFile, setLstmFile, ["text/csv"])}
              </CardContent>
            </Card>
          </TabsContent>
          <TabsContent value="vit">
            <Card>
              ...
              <CardContent>
                {renderFileUpload("ViT Image", "image/*", <ImageIcon className="h-6 w-6" />, vitFile, setVitFile, ["image/jpeg", "image/png", "image/svg+xml"])}
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
